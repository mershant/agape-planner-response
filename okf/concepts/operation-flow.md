...
5. Normal Planner content streams into the native reasoning disclosure. The
   Response does not start until nonblank Planning completes.
6. SillyTavern assembles its normal active Response prompt. Exact Planning is
   appended as the final `system` message under every preset prompt.
7. The chosen Response connection streams normal content into the same native
